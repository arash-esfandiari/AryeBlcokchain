...
class TxBlock(CBlock):
    ...
    def is_valid(self):
        ...
        for tx in self.data:
            if not tx.is_valid():
                return False
        total_in, total_out = self.count_total()
    ...
    def good_nonce(self):
        ...
        if this_hash[:leading_zeros] != bytes(
            "".join(["\x4f" for i in range(leading_zeros)]), "utf8"
        ):
            return False
...
if __name__ == "__main__":
    ...
    B5.addTx(Tx8)
    if not B5.is_valid():
        print("Success! Greedy minor detected")
    else:
        print("ERROR! Greedy minor undetected")
